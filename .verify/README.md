...
2. **Every subsystem rolls its own dice.** `ZS.rng32(seed ^ something)` —
   ...
   `Math.random()` moves the village's own random stream and *changes the
   weather at home*; that is a bug, and it is invisible unless you know to
...
4. Only then look for the cause. Measure before guessing: `ZS_TRACE=1`
   (day, souls, food, wood, stone, buildings) shows *where* a run dies
   long before it shows *why*.

**Known truth, not a bug:** `play.js 20` fails roughly two runs in three
on the current build *and* on the base commit it grew from. The death
...
- `node_modules` is not in the repo. `npm i` first, or the oxc bins are
  missing.
- The harness's `press()` takes an element *name* (`"panel"`), an action
